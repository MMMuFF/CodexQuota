...
> v0.8.4 新增跟随系统的中英文 App 界面，并修复短昵称旁额度过早缩成百分比的问题。完整变更见[更新说明](docs/releases/v0.8.4.md)。
...
- App 跟随系统首选语言：中文系统显示简体中文，其他语言显示英文；也可在 macOS 的单个 App 语言设置中指定，改后重新打开应用
...
底栏有语音等新增按钮时，额度会按其实际宽度让位，包括带文字的按钮。优先读取账户控件内头像与昵称的可见内容边界，回收短昵称后多余的留白；无法可靠取得内容边界时保守保护账户区域。如果辅助功能没有单独返回麦克风，仍会在问号左侧保留一个按钮位。

空间较小时依次使用 `46%·9/15·5天` → `46%·9/15` → `46%`，完全放不下时隐藏；拉宽后恢复完整文字，悬停详情始终保留完整信息。英文也使用同样的分级规则。CodexQuota 不会新增或开启 Codex 的语音入口，该入口是否显示由 Codex 自身决定。
...
检查覆盖额度解析、本机时区/跨日/夏令时、公共公告与预测语义、HTTP 缓存/限流/凭据隔离、账户隔离、重置券安全策略、窗口定位几何、后台显示策略、分区布局和实际组件渲染。包含短昵称与带文字语音按钮、渐进式文字精简、进度竖线、预测强度、悬停防闪烁回归，并在独立中文和英文测试进程中验证界面，不修改系统语言设置。AppKit 测试用模拟定位器和测试进程自有窗口验证，不读取真实 Codex 辅助功能界面；不覆盖真实辅助功能授权、TCC、Gatekeeper、跨应用/Space 切换及完整交互。
