...
> 此 README 展示当前源码中的新版分区详情、公共公告与自动时区功能。已发布安装包包含哪些功能，请以对应 Release 说明为准。
...
当前包括 60 项核心检查与 14 项 AppKit 检查，覆盖额度解析、本机时区/跨日/夏令时、公共公告与预测语义、HTTP 缓存/限流/凭据隔离、账户隔离、重置券安全策略、窗口定位几何、后台显示策略、分区布局和实际组件渲染。AppKit 测试用模拟定位器和测试进程自有窗口验证层级、跟随及隐藏恢复，不读取真实 Codex 辅助功能界面；不覆盖真实辅助功能授权、TCC、Gatekeeper、跨应用/Space 切换及完整交互。
