...
当前包括 55 项核心检查与 7 项 AppKit 检查，覆盖额度解析、日期格式、账户隔离、重置券安全策略、窗口定位几何、后台显示策略和实际组件渲染。AppKit 测试用模拟定位器和测试进程自有窗口验证层级、跟随及隐藏恢复，不读取真实 Codex 辅助功能界面；不覆盖真实辅助功能授权、TCC、Gatekeeper、跨应用/Space 切换及完整交互。
